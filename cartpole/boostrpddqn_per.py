...
class DQNAgent:
    ...
    def act(self, state, rand_index):
        ...
        with torch.no_grad():
            ...
            for i, qvalue in enumerate(self.model.forward(state)):
                Q_values[i, :] = qvalue.squeeze().cpu().numpy().reshape(1, self.action_size)            
            return (self.model.forward(state)[rand_index].squeeze().cpu().numpy() + 5*Q_values.std(axis=0)).argmax()  # returns action
